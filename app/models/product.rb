class Product < ActiveRecord::Base
  acts_as_paranoid
  scope :keyword_with, ->(keyword) { where("products.caption like ? or products.sub_caption like ? or products.itemcode like ?", "%#{keyword}%", "%#{keyword}%", "%#{keyword}%") unless self.nil? || keyword.blank? }
  scope :cates_with, ->(cate_ids) { joins(:categories).where("category_id in (?)", cate_ids) unless self.nil? || cate_ids.blank? }
